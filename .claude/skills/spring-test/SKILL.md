...
| `capture_sequence({unitId|unitIds|def|position|area|spawn, frames?, everyNthSimFrame?, mode?, simSpeed?, name?, outDir?, …})` | **Film a manoeuvre → N images on disk, in one call.** Same subject selectors and auto-framing as `capture_subject`, re-framed before every shot. `mode:"step"` (default) advances the sim by `sim_step` between shots so the spacing is EXACT however long each capture took; `mode:"realtime"` slows the sim and bursts browser-side (hard ~6.5 s ceiling — the relay abandons an evaluation at 8 s). N shots of the same instant comes back `UNUSABLE`, not as a film. |

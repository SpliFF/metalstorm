...
--
-- customparams.combat_model (PLAN-macro-combat.md):
--   "statistical" → simplified volley resolution when the engine path lands
--   "ballistic"   → keep full projectile sim (arcs / interception matter)
-- Until then everything runs the faithful ballistic path.
...
local function family(prefix, base, scales)
    for s, o in ipairs(scales) do
        ...
        w.customparams.combat_model = w.customparams.combat_model
            or base.customparams.combat_model
        defs[prefix .. '_S' .. s] = w
    end
end
...
family('MS_MG', {
    ...
    customparams = { combat_model = 'statistical' },
}, {
...
family('MS_RAILGUN', {
    ...
    customparams = { combat_model = 'ballistic' },
}, {

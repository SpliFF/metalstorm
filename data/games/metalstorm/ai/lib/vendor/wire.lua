-- lib/vendor/wire.lua — VERBATIM COPY of LuaRules/Gadgets/parley/wire.lua (the
-- RecvLuaMsg query-string codec). The AI's only write into synced Lua is
-- AI.sendMessage(opaque string) -> gadget:RecvLuaMsg, and the guidance gadget
-- decodes with this codec, so an AI must encode with it too. The sandbox
-- forbids requiring the original; lib/tests/vendor_drift_spec.lua asserts
-- byte-identity below the SPLIT line (same guard ai/strategos/wire.lua uses).
--
-- Original follows.
--

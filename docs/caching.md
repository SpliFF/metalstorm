...
Every build generates a unique stamp at build time (regenerated by the
`generate_build_stamp` CMake target on every `cmake --build`, not just on
configure — see `cmake/GenerateBuildStamp.cmake`):
...
The stamp changes on every `cmake --build` (it re-reads `git rev-parse --short HEAD` each time, not just at configure), so redeploying with a fresh build automatically invalidates all cached assets.
...
1. Build the server (`cmake --build build/debug`)
2. The build's `generate_build_stamp` step regenerates the stamp from current HEAD

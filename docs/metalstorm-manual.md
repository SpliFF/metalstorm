...
3. **Stage an attack** — commit at least one transport carrying at least one squad to a POI your faction doesn't hold (`POST /api/world/staging/commit`). The defender needs no transport. Transit time is priced by POI-graph edge weights (60 000 world-ms per km): **the attacker's world-days in transit are the defender's real hours of warning.** Late commits may join during the staging window.
4. **Force goes into escrow** (`WorldEscrow`): `committed → engaged → settled`. Escrowed force is unavailable to the world; the battle consumes it via the `world_commit` modoption and reports back only through `war_outcome`. Outcomes per side: `held | withdrew | routed | annihilated` — an annihilated expedition transfers **a quarter of its materiel to the victor** (the comeback valve); holding pays treasury spoils.
...
6. **Claim ground.** Conquest is an **explicit claim act** (`WorldConquest`, decided 2026-08-27): POI ownership transfers at war end only to a winning-side faction that filed a claim on that POI **before the war ended** (filing costs 25 world authority). No valid claim → the owner keeps it. **The defender's shield:** if the current owner is on the winning side, the POI never changes hands and an allied claim cannot snipe it. Earliest open winning claim wins; losing claims refund 50%; claims expire after 30 world-days.
7. **The economy accrues in absentia** — +2 treasury per world-day per POI held, with decay; event-sourced, so it catches up in one closed-form step after any outage. Pausing the world (`world_pause_ledger`) freezes accrual, seasons, and claim expiry for free.
8. **Seasons** roll every 14 world-days: a digest of settlements and economy per faction is archived and a new season opens — no balances or ownership are touched. Digests are served at `GET /api/world/seasons[/{n}]`.
...
Rank (`WorldStats`) weighs commanders, authority, **POIs held (25 pts each)**, artifacts, money, resources, units. Commander action capacity recharges over 24 **real** hours — an admin pause can't widen anyone's budget.
...
- **World holdings aren't seeded yet:** committing force a faction doesn't have is refused by nothing; the escrow ledger opens negative until a world-holdings milestone lands.

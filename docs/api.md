...
| `/api/world/claims/file` | POST, token | Body `{poi}`. Charges the caller's world authority (`claimPoiCost`, 25). 409 `already_owner`/`already_claimed`, 404 `no_poi`/`no_faction`, **400** `insufficient_authority` (with `have`/`need` — note: 400, not 403 as the founding route answers; see the 2026-09-10 review), 500 `db_error`. → `{ok, claim}` |

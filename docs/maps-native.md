...
> **Status (2026-07-27 realistic-maps directive):** the shipped
> `meridian_basin` package is now produced by the **terragen** pipeline
> (`tools/mapgen/meridian2.py` — erosion, biomes, baked albedo, splat
> detail; see [maps/generation.md](maps/generation.md)), which replaced
> `meridian.py`'s banded palette-tile texturing described below. This doc
> remains the reference for the **layout/region/validator half** of the
> process (layout JSON → region graph → E1 validation → civilian/convoy
> data → scenario), which terragen generators reuse unchanged, and for
> `meridian.py` as a legacy texturing reference.

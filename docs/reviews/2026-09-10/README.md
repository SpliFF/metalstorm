...
| 6 | ai-framework | [ai-framework.md](ai-framework.md) | `ai/lib/` reusable AI-player library (engine facade, picture, regions, authority preview, directives, rate-limited actuator, scheduler, reporter, fake engine for specs, vendored spec copies with a drift test); `ai/garrison/` second AI with three profiles; engine-surface census. **Queue closed 2026-09-17**: `tools/ai-eval` (`make test-ai-eval`, baseline-gated, no-op control), `docs/ai-players.md`, garrison `brain_spec`. | — (see the report's closing note for what the harness found and who owns it) |
| 7 | mcp-control | [mcp-control.md](mcp-control.md) | `end_game` on a hibernated room signalled pid 0 (own process group) — fixed; ended rooms no longer route to a port squatter; deadlines on every fetch; schema drift fixed; token redaction; `tools.js` catalogue; guidance wire encoder, SSE parser, fengari-tested Lua snippets; `world_*`/`ai_*`/`nl_command` tools implemented (incl. the `world_status`/`world_commit` pair world-design asked for); `self-check.mjs` schema/handler drift gate; generated `docs/mcp-tools.md`. | **Queue closed 2026-09-17.** Remaining: a `window.test.nl()` hook (lane 9/10), and the 7 bake-dependent tests that should `t.skip`. |
...
6. ~~`tools/ai-eval` + `docs/ai-players.md`; garrison brain spec.~~ **Done
   2026-09-17.** Next for this lane: a recorded-snapshot fixture from a real
   headless run (every fixture today is synthetic), and the strategos gaps the
   harness surfaced (below).

...
export class WorldScreen {
    ...
    private claimsHtml(poi: WorldPoi): string {
        ...
        const rule = '<div class="world-sub">Ownership moves at mission end only to a winning-side faction with a claim filed before the mission ended; the earliest claim wins, and a defended place never changes hands.</div>';        return head + control + this.messageHtml('poi', 'claim') + rule;
    }
    ...
    private showTooltip(poi: WorldPoi, at: { x: number; y: number }): void {
        ...
        const what = poi.battleStatus === 'active' ? 'battle in progress'
            : poi.battleStatus === 'staging' ? 'mission staging'            : poi.mapId ? 'battle map' : 'world only';

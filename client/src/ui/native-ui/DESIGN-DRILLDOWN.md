...
| `#hud-selection` — "Selected: unit 42" / "Selected: 3 units" | **already folded** — `style="display:none"` in `hud.html` and NO code path shows it | Superseded by the rung-1 focus chip, which says the same thing with a *name*. U0–U4 carried this row as "still open; the per-game switch is the whole job"; the 2026-09-10 review checked and found it in the same state U3 found `#hud-help`: shipped hidden, never shown. `updateHUD` still wrote its text every frame into the hidden node — that per-frame write is now guarded (`hud.ts` write-if-changed). No per-game switch is needed. |

 ...
 *
 * FIDELITY notes (called out per AGENTS.md):
...
export interface CursorModeReq {
  name: string | null;
  css: string;
}
...
export class WorkerCommandModes {
  ...
  cancelAll(): void {
    ...
    if (this.waypointDrag) {
      ...
      if (this.waypointDrag.markerMesh)
        this.waypointDrag.markerMesh.isVisible = true;
  ...
  pointerDown(
    ...
    if (ctrl && !shift && !alt && this.tryRevokeWaypointAt(x, y, viewId))
      return true;

    // Shift+down over a waypoint marker → begin a reposition drag.
    if (shift && !ctrl && !alt && this.tryStartWaypointDrag(x, y, viewId))
      return true;
    ...
    if (this.pendingCmd === CMD.ATTACK || this.pendingCmd === CMD.AREA_ATTACK) {
      ...
      if (groundPos && this.opts.getSelection().length > 0) {
        this.startAreaAttackDrag(
          groundPos,
          shift,
          x,
          y,
          this.pendingCmd === CMD.AREA_ATTACK,
        );
      }
      return true;
    }
  ...
  pointerMove(
    ...
    if (this.waypointDrag) {
      this.updateWaypointDrag(x, y, viewId);
      return;
    }
    if (this.areaAttackDrag) {
      this.updateAreaAttackDrag(x, y, viewId);
      return;
    }
  }
  ...
  pointerUp(
    ...
    if (button !== 0) return false;
    if (this.waypointDrag) {
      this.commitWaypointDrag(x, y, viewId);
      return true;
    }
    if (this.areaAttackDrag) {
      this.commitAreaAttackDrag(x, y);
      return true;
    }
    if (this.pendingCmd !== null) {
      const moved =
        Math.hypot(x - this.pressX, y - this.pressY) >= DRAG_THRESHOLD_PX;
      if (!moved) {
        const groundPos = this.pickGroundAt(x, y, viewId);
        if (groundPos)
          this.resolvePendingCommandAt(groundPos, (mods & 1) !== 0);
  ...
  private updateAreaAttackDrag(x: number, y: number, viewId: number): void {
    ...
    if (!groundPos) return;
    const r = Math.hypot(
      groundPos.x - drag.centerX,
      groundPos.z - drag.centerZ,
    );
    drag.radius = Math.min(
      AREA_ATTACK_MAX_RADIUS,
      Math.max(AREA_ATTACK_MIN_RADIUS, r),
    );
    this.renderAreaAttackRing();
  }
      ...
      if (drag.isAreaOnly || moved) {
        ...
        if (nearest.id >= 0) {
          ...
          this.commandBuffer.issueImmediate(
            CMD.ATTACK,
            sel,
            [drag.centerX, drag.centerY, drag.centerZ],
            opts,
          );
        }
      }
  ...
  private renderAreaAttackRing(): void {
    ...
    const ring = MeshBuilder.CreateTorus(
      "area-attack-ring",
      { diameter: drag.radius * 2, thickness, tessellation: tess },
      this.scene,
    );
    ring.scaling.y = 0.15;
  ...
  private tryStartWaypointDrag(
    ...
    this.waypointDrag = {
      unitId: hit.meta.unitId,
      tag: hit.meta.tag,
      cmdId: hit.meta.cmdId,
  ...
  private pickWaypointMarker(
    cssX: number,
    cssY: number,
    viewId: number,
  ): {
    mesh: Mesh;
    meta: { unitId: number; tag: number; cmdId: number };
  } | null {
    ...
    const pick = this.scene.pick(
      cssX * dpr,
      cssY * dpr,
      (m) => m.isPickable && m.name.startsWith("waypoint-marker-"),
      false,
      camera,
    );
    if (!pick?.hit || !pick.pickedMesh) return null;
    const meta = (
      pick.pickedMesh.metadata as {
        waypoint?: { unitId: number; tag: number; cmdId: number };
      } | null
    )?.waypoint;
    if (!meta) return null;
  ...
  handleOrderKey(code: string, mods: number): boolean {
    ...
    switch (code) {
      // ---- Instant orders ----
      case "KeyS":
        this.commandBuffer.issueImmediate(CMD.STOP, ids, []);
        return true;
      case "KeyW":
        this.commandBuffer.issueImmediate(CMD.WAIT, ids, [], queue);
        return true;
      case "KeyH":
        this.commandBuffer.issueImmediate(CMD.MOVE_STATE, ids, [0]);
        return true;
      case "KeyQ":
        this.cycleFireState(ids);
        return true;
      case "KeyI":
        this.commandBuffer.issueImmediate(CMD.IDLEMODE, ids, [-1]);
        return true;
  ...
  private cycleFireState(ids: number[]): void {
    ...
    this.commandBuffer.issueImmediate(CMD.FIRE_STATE, ids, [
      this.fireStateCycle,
    ]);
  }
  ...
  private pickNearestEntityAt(groundPos: Vector3): {
    id: number;
    team: number;
  } {
...
export function cursorForCommand(cmd: number | null): CursorModeReq {
  switch (cmd) {
    case CMD.ATTACK:
      return { name: "Attack", css: "crosshair" };
    case CMD.AREA_ATTACK:
      return { name: "Area attack", css: "crosshair" };
    case CMD.FIGHT:
      return { name: "Fight", css: "crosshair" };
    case CMD.MANUALFIRE:
      return { name: "ManualFire", css: "crosshair" };
    case CMD.PATROL:
      return { name: "Patrol", css: "cell" };
    case CMD.MOVE:
      return { name: "Move", css: "move" };
    case CMD.UNLOAD_UNITS:
      return { name: "Unload units", css: "move" };
    case CMD.LOAD_UNITS:
      return { name: "Load units", css: "pointer" };
    case CMD.GUARD:
      return { name: "Guard", css: "pointer" };
    case CMD.REPAIR:
      return { name: "Repair", css: "pointer" };
    case CMD.RECLAIM:
      return { name: "Reclaim", css: "pointer" };
    case CMD.CAPTURE:
      return { name: "Capture", css: "pointer" };
    case CMD.RESURRECT:
      return { name: "Resurrect", css: "pointer" };
    case CMD.SELFD:
      return { name: "SelfD", css: "crosshair" };
    case CMD.WAIT:
      return { name: "Wait", css: "progress" };
    default:
      return { name: null, css: "" };
  }
}

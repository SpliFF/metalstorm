 ...
 * Lua/JS callback keeps working — faithfully, per CLAUDE.md — but only
...
export class EntityFxFence {
    ...
    constructor(private readonly frameBudgetMs: number = DEFAULT_FRAME_BUDGET_MS) {
     ...
     * `performance.now`.
     */
    run(defName: string, entityId: number, distanceToCamera: number, fn: () => void, now: () => number = () => performance.now()): boolean {

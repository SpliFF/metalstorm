...
export function computeBuildPositions(
    ...
    for (let zi = 1; zi < zCount; zi++) out.push([sx + xDir * (xCount - 1) * stepX, sz + zDir * zi * stepZ]); // right
    for (let xi = xCount - 2; xi >= 0; xi--) out.push([sx + xDir * xi * stepX, sz + zDir * (zCount - 1) * stepZ]); // bottom R→L
    for (let zi = zCount - 2; zi >= 1; zi--) out.push([sx, sz + zDir * zi * stepZ]); // left
    return out;
}
...
export class WorkerBuildPlacement {
    ...
    private buildDrag: {
        ...
        shift: boolean; mode: 'line' | 'rect' | 'hollow';
    ...
    private pendingBuilds: Array<{
        ghost: TransformNode; defId: number; gx: number; gz: number; owners: Set<number>;
    }> = [];
    ...
    startBuildPlacement(defId: number, mods: { shift?: boolean; ctrl?: boolean } = {}): void {
        ...
        if (ghostIsBox) {
            this.entityRenderer.requestModel(defId, LoadPriority.P0)
    ...
    pointerUp(x: number, y: number, button: number, _mods: number, viewId = 0): boolean {
        if (!this.placement || button !== 0) return false;
        if (this.buildDrag) { this.commitBuildDrag(x, y, viewId); return true; }
        const groundPos = this.pickGroundAt(x, y, viewId);
    ...
    private startBuildDrag(groundPos: Vector3, mods: number, downX: number, downY: number): void {
        const alt = (mods & 4) !== 0;
        const ctrl = (mods & 2) !== 0;
        ...
        this.buildDrag = {
            startX: groundPos.x, startZ: groundPos.z, startY: groundPos.y,
            downX, downY, shift: (mods & 1) !== 0, mode,
            previewGhosts: [], previewPositions: [],
        };
    }
    ...
    private updateBuildDrag(x: number, y: number, viewId: number): void {
        ...
        if (p.isMex) { this.updateGhost(x, y, viewId); return; }

        const positions = computeBuildPositions(
            drag.startX, drag.startZ, groundPos.x, groundPos.z,
            xsize, zsize, this.opts.getBuildSpacing(), drag.mode);

        if (positions.length <= 1) { this.updateGhost(x, y, viewId); return; }
        ...
        for (let i = 0; i < limit; i++) {
            ...
            try { ghost = this.entityRenderer.createGhostMesh(p.defId, `build-drag-ghost-${i}`); }
            catch { ghost = null; }
            if (!ghost) ghost = makeBoxGhost(this.scene, p.footprintX, p.footprintZ, p.defaultEmissive);
    ...
    private commitBuildDrag(x: number, y: number, viewId: number): void {
        ...
        const dragged = Math.hypot(x - drag.downX, y - drag.downY) >= DRAG_THRESHOLD_PX;
        ...
        const defId = p.defId;
        const facing = 0;
        const batch: Array<{ commandId: number; unitIds: number[]; params: number[]; options?: number }> = [];
        for (let i = 0; i < positions.length; i++) {
            const [px, py, pz] = positions[i];
            batch.push({
                commandId: -defId, unitIds: builders, params: [px, py, pz, facing],
        ...
        for (const [px, , pz] of positions) this.spawnPendingBuildGhost(defId, px, pz, builders);
    ...
    private promoteGhostToPending(defId: number, x: number, z: number, owners: number[]): void {
        ...
        this.pendingBuilds.push({ ghost: p.ghost, defId, gx: Math.round(x), gz: Math.round(z), owners: new Set(owners) });
    }
    ...
    private spawnPendingBuildGhost(defId: number, x: number, z: number, owners: number[]): void {
        ...
        if (!p) return;
        let ghost: TransformNode | null = null;
        try { ghost = this.entityRenderer.createGhostMesh(defId, `build-pending-${defId}-${x},${z}`); }
        catch { ghost = null; }
        if (!ghost) ghost = makeBoxGhost(this.scene, p.footprintX, p.footprintZ, PENDING_EMISSIVE);
        ...
        this.pendingBuilds.push({ ghost, defId, gx: Math.round(x), gz: Math.round(z), owners: new Set(owners) });
    }
    ...
    onCommandQueuesUpdated(queues: ReadonlyArray<UnitCommandQueueInfo>): void {
        ...
        for (const q of queues) {
            ...
            for (const o of q.orders) {
                if (o.cmdId < 0 && o.params.length >= 3) {
                    set.add(`${-o.cmdId}@${Math.round(o.params[0])},${Math.round(o.params[2])}`);
                }
            }
        ...
        for (const pb of this.pendingBuilds) {
            ...
            for (const owner of pb.owners) {
                if (unitToBuilds.get(owner)?.has(key)) { stillQueued = true; break; }
            }
    ...
    private trySwapGhostMesh(defId: number): void {
        ...
        try {
            meshGhost = this.entityRenderer.createGhostMesh(defId, `build-ghost-${defId}`);
        } catch (err) {
        ...
        this.tintGhost(meshGhost, false, p.valid ? p.defaultEmissive : INVALID_EMISSIVE);
        p.ghost.dispose();
        p.ghost = meshGhost;
        p.ghostIsBox = false;
    }

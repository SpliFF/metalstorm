...
/// Accumulator for one phase (or the whole frame). A free-standing struct,
/// not nested in SimFrameProfiler — a nested type's default member
/// initializers aren't visible yet when the enclosing class declares a
/// same-scope array of it (`default_member_initializer_not_yet_parsed`).
 ...
 *
 * Single-threaded by construction (sim thread only, same as
 * LuaCallInProfiler) — no locking. Disabled by default; per-frame cost
 * when off is 3 bool loads + 3 skipped spring_now() pairs.
 */
...
public:
	...
	static void RecordPhase(Phase p, double us) {
		PhaseStats& e = phases_[p];
		e.samples++;
		e.totalUs += us;
		if (us > e.maxUs)
			e.maxUs = us;
	...
	static void RecordFrame(double us) {
		frame_.samples++;
		frame_.totalUs += us;
		if (us > frame_.maxUs)
			frame_.maxUs = us;
	...
	static const PhaseStats& GetPhase(Phase p) { return phases_[p]; }
	static const PhaseStats& GetFrame() { return frame_; }
	...
	static std::string Report() {
		...
		ss << "sim profile: " << (enabled_ ? "on" : "off")
		   << ", " << frame_.samples << " frame(s) sampled\n";
...
private:
	...
	static inline PhaseStats phases_[Phase_Count];
	static inline PhaseStats frame_;
};

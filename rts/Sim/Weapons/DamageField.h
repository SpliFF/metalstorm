...
// affected (CLAUDE.md "Faithful Recoil Reproduction": a called-out, opt-in
...
public:
	...
	uint32_t Create(uint8_t shape, const float3& center, float radius, float halfZ,
	...
	std::vector<DamageField> CollectDamageTicks(int frame, std::vector<uint32_t>& expiredOut);
...
private:
	...
	DamageFieldEventData MakeEvent(const DamageField& f, uint8_t kind, int currentFrame) const;
};

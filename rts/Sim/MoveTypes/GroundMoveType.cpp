	...
	if (owner->GetTransporter() != nullptr) {
		...
		if (progressState == Active) {
			...
			if (wantRepath) {
				...
				// SlowUpdate.
				float curDist = std::floor(currWayPoint.distance2D(owner->pos) * 10.f) / 10.f;
...
bool CGroundMoveType::CanSetNextWayPoint(int thread) {
	...
	if (!allowSkip) {
		...
		if (!skipRequested) {
			...
			// #ifdef PATHING_DEBUG
			// if constexpr (DEBUG_DRAWING_ENABLED)
			// {
			// 	bool printMoveInfo = (selectedUnitsHandler.selectedUnits.size() == 1)
			// 		&& (selectedUnitsHandler.selectedUnits.find(owner->id) != selectedUnitsHandler.selectedUnits.end());
			// 	if (printMoveInfo) {
...
{
	// FPS control is a client-side feature; not supported on the headless server
	return false;
}

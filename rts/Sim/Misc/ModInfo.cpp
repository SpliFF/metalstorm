...
/// Try loading the game's top-level `game.config.lua` (auto-
/// generated by gameconverter, wraps the legacy `modinfo.lua` via
/// `VFS.Include`). Used here to read game-identity flags the sim
/// cares about — currently just `legacyCoordSystem`. Returns
/// nullptr if neither `game.config.lua` nor `game.config.json`
/// is present.
static std::unique_ptr<LuaParser> TryLoadGameConfig()
{
	const std::string baseName = "game";
...
void CModInfo::Init(const std::string& /*modFileName*/)
{
	...
	// Game-level identity flags live in `<game>/game.config.lua`,
	// which gameconverter generates as a thin wrapper around the
	// author's `modinfo.lua`. We read coordinate-system mode
	// (`legacyCoordSystem`) from there before the rules pass.
	if (auto gameCfg = TryLoadGameConfig()) {
		const LuaTable& root = gameCfg->GetRoot();
		legacyCoordSystem = root.GetBool("legacyCoordSystem", false);
		LOG_L(L_NOTICE, "[ModInfo] legacyCoordSystem=%s",
			legacyCoordSystem ? "true" : "false");
	}

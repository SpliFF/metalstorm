...
S3DModel* SolidObjectDef::LoadModel() const
{
	...
	// origins. The data comes from the document-level SPRINGRTS_geometry
	// extension embedded in `<stem>.gltf` by tools/modelimporter — the
	// `.gltf` is the canonical record; legacy `.config.lua` /
	// `.config.json` sidecars are no longer consulted.
	if (model != nullptr)
		return model;
	if (modelName.empty())
		return nullptr;
	...
	// CFileHandler resolution is case-insensitive, so this works even
	// when def files reference models with mixed case while the on-disk
	// file is lowercase.
	namespace fs = std::filesystem;
	const std::string stem = fs::path(modelName).stem().string();
	const std::string gltfName = stem + ".gltf";

	if (!CFileHandler::FileExists(gltfName, SPRING_VFS_MOD)) {
		SLOG(SPRING_LOG_NOTICE,
			"[SolidObjectDef::LoadModel] no '%s' in VFS for '%s'",
			gltfName.c_str(), modelName.c_str());
		return nullptr;
	}

	const fs::path resolved = CFileHandler::GetFileAbsolutePath(gltfName, SPRING_VFS_MOD);
	const std::string basePath = (resolved.parent_path() / stem).string();

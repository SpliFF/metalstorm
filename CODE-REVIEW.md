...
`rts/server_main.cpp:1826–1834` — any authenticated client (player or spectator) can push arbitrary code to `LuaExecEngine` in `LuaRules`/`server` scope: spawn units, pause, change speed, run Lua. The HTTP `/api/exec` path validates a token but also never checks role. This directly contradicts CLAUDE.md's stated design ("privileged actions… gated behind a permissions system").
...
`CMakeLists.txt:36` globally defines `NOT_USING_CREG` while CLAUDE.md's design table says creg is retained for save/load. If the macro stubs creg out, snapshots will be silently empty. Audit and reconcile (code or docs).
...
6. **Housekeeping (Q3–Q5, B4, B7, B8):** opportunistic; good "cheap model + parallel agents" cleanup tasks per CLAUDE.md.

---

*Generated by automated review (3 parallel agents over server C++, client TS, build/hygiene; high-severity findings spot-verified against source). Line numbers will drift — treat them as anchors, not gospel.*

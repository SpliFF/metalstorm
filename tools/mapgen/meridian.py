...
HM_H = MAPY + 1
# 512 (each tile = 4 squares = 32 elmos)
TILES_X = MAPX // 4
TILES_Z = MAPY // 4
...
ELEVATION = {
    ...
    # infantry-only (32-45deg)
    "west_scarp_n": 1230, "west_scarp_s": 1230,
    # veh/heavy_restricted (24-32deg)
...
def bbox_weight(x, z, bbox, margin):
    ...
    if dx > 0.0 or dz > 0.0:
        ...
        dist = -min(x - bbox["x0"], bbox["x1"] - x, z -
                    bbox["z0"], bbox["z1"] - z)  # negative = inside
...
def slope_deg_at(hm, vx, vz):
    ...
    z0 = max(vz - 1, 0)
    z1 = min(vz + 1, HM_H - 1)
    dhdx = (hm[vz * HM_W + x1] - hm[vz * HM_W + x0]) / \
        ((x1 - x0) * SQUARE_SIZE or 1)
    dhdz = (hm[z1 * HM_W + vx] - hm[z0 * HM_W + vx]) / \
        ((z1 - z0) * SQUARE_SIZE or 1)
...
def emit_regions_lua(layout, out_path):
    ...
    regions.sort(key=lambda r: (0 if r.get(
        "declare_before") else 1, order[r["key"]]))
...
def rgb565(r, g, b):
    # keep >0 so color0 != 0 (lets us force color0>color1)
    r = max(1, min(255, r))
...
def solid_tile_record(rgb):
    """One SMT small-tile record: mip0(64 blocks) + mip1(16) + mip2(4) + mip3(1)."""
    block = solid_dxt1_block(rgb)
    return block * (64 + 16 + 4 + 1)
...
# re-use of shared art direction; called out here per AGENTS.md's
# "never deviate from Recoil/shared-direction silently" rule.
# ============================================================

BAND_COLOR = {
    # atlas row3 col0 "concrete grey" — build zones / passes
    ("flat", None): (0x9C, 0x9A, 0x93),
    # atlas row1 col2 "worn steel" — overlook slopes
    ("veh", None): (0x6B, 0x6F, 0x73),
    # atlas row3 col2 "civilian tan" — scarp walls
    ("infantry", None): (0xC6, 0xB3, 0x93),
    # atlas row3 col3 "ground-contact dark" — crests
    ("cliff", None): (0x2B, 0x2B, 0x2C),
    # map-specific water blues — no atlas swatch exists for water
    (None, "ford"): (92, 132, 150),
...
def color_for(h, deg):
    ...
    if depth > 0.0:
        return BAND_COLOR[(None, water_band(depth))]
    return BAND_COLOR[(slope_band(deg), None)]


def build_tile_palette():
    """Fixed 8-entry palette: 4 slope bands + 4 water bands."""
    order = [
        ("flat", None), ("veh", None), ("infantry", None), ("cliff", None),
        (None, "ford"), (None, "shallow"), (None, "deep"), (None, "channel"),
    ]
    palette = [BAND_COLOR[k] for k in order]
    index_of = {k: i for i, k in enumerate(order)}
    return palette, index_of


def band_key_for(h, deg):
    depth = water_depth_at(h)
    if depth > 0.0:
        return (None, water_band(depth))
    return (slope_band(deg), None)
...
def build_binary(layout):
    ...
    palette, index_of = build_tile_palette()

    # Tile index: one entry per (TILES_X x TILES_Z) 32-elmo tile, sampled at
    # the tile's center heightmap vertex.
    tile_index = bytearray(TILES_X * TILES_Z * 4)
    ti = 0
    for tz in range(TILES_Z):
        vz = min(tz * 4 + 2, HM_H - 1)
        for tx in range(TILES_X):
            vx = min(tx * 4 + 2, HM_W - 1)
            h = hm[vz * HM_W + vx]
            deg = slope_deg_at(hm, vx, vz)
            band = band_key_for(h, deg)
            struct.pack_into("<i", tile_index, ti, index_of[band])
            ti += 4
    ...
    industrial_keys = {r["key"]
                       for r in layout["regions"] if "industrial" in r["tags"]}
    industrial_bboxes = [r["bbox"]
                         for r in layout["regions"] if r["key"] in industrial_keys]
    ...
    # Minimap: 1024x1024 DXT1, one solid block per 4x4-texel cell, sampled
    # from world position (band colour — same palette as the terrain tiles).
    mm_blocks_per_row = 1024 // 4
    minimap = bytearray(mm_blocks_per_row * mm_blocks_per_row * 8)
    mi = 0
    for by in range(mm_blocks_per_row):
        z = (by + 0.5) / mm_blocks_per_row * MAP_SIZE
        vz = min(int(z / SQUARE_SIZE), HM_H - 1)
        for bx in range(mm_blocks_per_row):
            x = (bx + 0.5) / mm_blocks_per_row * MAP_SIZE
            vx = min(int(x / SQUARE_SIZE), HM_W - 1)
            h = hm[vz * HM_W + vx]
            deg = slope_deg_at(hm, vx, vz)
            rgb = color_for(h, deg)
            minimap[mi:mi + 8] = solid_dxt1_block(rgb)
            mi += 8
...
def write_smf(path, data):
    ...
    num_tiles = len(data["palette"])

    # numTileFiles, totalTiles
...
def write_smt(path, palette):
    header = bytearray(32)
    header[0:16] = b"spring tilefile\0"
    struct.pack_into("<i", header, 16, 1)          # version
    struct.pack_into("<i", header, 20, len(palette))  # numTiles
    with open(path, "wb") as f:
        f.write(bytes(header))
        for rgb in palette:
            f.write(solid_tile_record(rgb))
...
def generate(out_dir):
    ...
    write_smt(smt_path, data["palette"])
    ...
    if args.selftest:
        ...
        for i in range(2):
            with tempfile.TemporaryDirectory() as td:
                ...
                hashes.append(
                    (hash_file(smf), hash_file(smt), hash_file(regions)))
        if hashes[0] == hashes[1]:
            print(
                "SELFTEST OK: two independent generator runs produced identical output.")
        else:

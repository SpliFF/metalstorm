"""check_ms_fishing_trawler — spec-level checks for the trawler build.

Buffer/bin agreement, bufferView bounds, spec pieces (boom) and the
idle sway clip present, clip channels resolve to real nodes.
"""
import json
import os
import sys
...
for stem in ('ms_fishing_trawler', 'ms_fishing_trawler_png'):
    ...
    if 'boom' not in gnames:
        fails.append(f'{stem}: SPRINGRTS_geometry missing piece boom')

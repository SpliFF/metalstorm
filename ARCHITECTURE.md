...
| `core/game-processor.ts` | **GP half** (WP2c). All `gp*` state + functions: Babylon `Engine`/`Scene`, interactive `RTSCamera`, `EntityRenderer`, `DefCache`, `PresentationClock`, FX renderers (projectile, CEG, CombatFX, build-beam, decal, feature), `Connection`, order overlays, `gpInit`/`gpConnect`/`gpLoadMap`/`gpShutdown`, scene-state + minimap feed, LuaUI `gpRunUiPass`/`gpBootLuaUI`, input-dispatch helpers, GW8 test harness. Imports from `lua-ui-host.ts` (one-way). |

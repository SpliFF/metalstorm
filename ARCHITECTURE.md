...
| `core/lua-widget-worker.ts` | LuaUI Web Worker entry: Fengari + OffscreenCanvas, runs widgets off the main thread (PLAN-widgets.md). **GW4 (c1–c4 + c5a landed)**: also the game-processor worker — on `gp:init` owns the Babylon `Engine` on the transferred `#game-canvas`, the WebTransport `Connection` + decoders, the terrain + lighting, the entity-renderer + def-cache + presentation-clock (units stream → interpolate → render from the worker), and (c5a) the weapon-FX / projectile / decal / build render modules (projectile + ceg runtime, fx-light-pool, distortion, muzzle-flare, combat-fx, build-beam, decal-overlay, feature-renderer) aged in the render loop at the sim-scaled FX delta. **c5b-1** adds the interactive RTS camera per view (DOM-free `rts-camera.ts` driven by `gp:*` input from the main-thread `camera-input.ts`; `gpViewCameras` map keyed by viewId for multi-view). **c5b-2** adds worker-side selection/pick/orders (`worker-selection.ts`): left-button click + drag-box select, right-click move/attack/guard sent over the worker's own connection, drag-box overlay posted to main. **c5b-3** adds the selection-driven order overlays (command-path / waypoint-marker / standing-order renderers) fed by `onUnitCommandQueues` + `onStandingOrders` + the worker selection set; shift-gated, with the standing-order show-allies localStorage pref lifted to main via `gp:config`. **c5c-1** adds the `gp:sceneState` feed (~10 Hz) → the HTML HUD (entity count / frame / selection / speed-pause). **c5c-2** adds the audio bridge: the worker resolves each SoundEvent's SoundRef against its def cache and posts `gp:audioSoundEvents` (+ `gp:audioMusic`) → main's AudioManager/SoundEventPlayer/MusicDirector (listener pose from the sceneState camera). **c5c-3** adds the minimap feed (`gpPostMinimapFeed` ~6 Hz → `gp:minimapFeed`: struct-of-arrays unit blips with fog filtered producer-side, the LOS bitmap when fresh, map dims + backdrop URL on the first feed → main-thread `Minimap`) + the `gfx.*` config push (worker seeds clientSettings from the `gp:init` snapshot before lighting builds, restores the FX-gating block, and applies live `gp:config` via `clientSettings.set` → notify) + `gp:focusWorld` (minimap click → worker camera). **c6-1a** boots the Fengari/chili LuaUI host on the **same** Babylon GL context (`init()` takes an optional `sharedGl` = `(gpEngine as any)._gl`; `gpBootLuaUI` at the end of `gpLoadMap` runs the full bootstrap; `gpRunUiPass` runs the screen-space `DrawScreen` pass after `scene.render()` with GL state save/restore + `wipeCaches`, no color-clear) — chili screen-space UI composites over the world. **c6-1b** feeds the in-worker Lua runtime from the worker's own sources: `gpDefCache.onUnitDefs`/`onWeaponDefs` → `unitDefMap`/`weaponDefMap` + `republishDefGlobals` (so `UnitDefNames` resolves — `gpBootLuaUI` `await`s a `gpDefsReady` promise before `init()` so ZK config files don't index nil at include-time); `onGameInfo` → `liveState.gameFrame`; `gpSelection.onSelectionChange` → `liveState.selectedUnitIds` + `dispatchSelectionChanged`; `onAuthenticated` → `liveState.identity`; `onEntityState`/`onEntityDestroy` → `applyEntityStateToLiveState`/`removeUnitFromLiveState` (units map + UnitCreated/UnitFinished/UnitDestroyed synth, extracted from the legacy message handlers); `gp:pointer*`/`gp:wheel`/`gp:key*` → `widgetHandler` Mouse/Key callins (`gpDispatch*` + CSS→device-px bottom-left Y-flip + DOM→Spring button + code→SDL keysym), with world selection suppressed when a widget consumes the press. **c6-2a** adds the world-space pass: the immediate GL renderer now uses 3D vertices (`lua-gl-immediate.ts` vec2→vec3, `gl.Vertex(x,y[,z])`), `gl.LoadMatrix('projection'/'view')` resolves the per-frame camera matrices the bridge caches, and `runFrame`'s `worldPass` runs `_G.DrawWorldPreUnit`/`_G.DrawWorld` before the `DrawScreen` block with depth-test enabled (camera matrices fed each frame from `scene.getViewMatrix()`/`getProjectionMatrix()`; scene coords == server world coords so a widget's `gl.Vertex(serverX,serverY,serverZ)` projects onto the world). **c6-2b** deletes the `worldGLCommands` no-op (Options X/Y/Z retired) and makes `ImmediateModeRenderer.flush()` run a **bound custom shader** (`gl.UseShader`) over its vertex stream instead of always using its built-in `uMVP` program: when an override is set it feeds the legacy fixed-function uniforms the GLSL `legacyGL2Shim` emits (`_legModelViewProjectionMatrix`, `_legModelViewMatrix`/`_legProjectionMatrix` when used, `_legTextureMatrix[0]` = identity, `tex0`) — the VAO loc 0/1/2 already match the shim's `_legVertex`/`_legColor`/`_legMultiTexCoord0`. This makes ZK world widgets that draw through their own GLSL render faithfully (the Map Edge Extension mirror shader: mirroring/curvature/brightness/alpha-fade now run, so the border is the transparent curved grid extension, not opaque full-bright wireframe tiles). `gl.UseShader(0)` after the world pass stops a leaked shader from bleeding into the DrawScreen chili pass. **GW4-c6 (and GW4) complete.** |

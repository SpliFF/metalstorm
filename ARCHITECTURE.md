...
| `core/input-manager.ts` | Click-to-select (ray cast), right-click-to-command, drag-box select, keyboard shortcuts. **GW4**: being relocated into the worker — selection/picking/orders land in c5b-2 (PLAN-game-worker.md). |
...
**Transport:** All HTTP endpoints support both HTTP/2 (h2c, cleartext) and HTTP/1.1. Game state streaming runs over **WebTransport (QUIC/HTTP-3)** via `WebTransportServer` (GW1–GW3 landed; PLAN-game-worker.md, PLAN.md Stage 0). The client discovers the endpoint via `GET /api/wt/info`. WebRTC is **fully removed** (GW7): `WebRTCServer.{h,cpp}`, libdatachannel, and the `/api/rtc/*` signaling are gone; `libspringapi` no longer links libdatachannel (`connectRtc` is an inert stub pending a WebTransport port). The remaining migration (GW4) relocates the connection + render core into the game-processor worker — c1–c4 + c5a + c5b-1 landed (connection, terrain, interpolated entities, the weapon-FX/projectile/decal/build render modules, and the interactive per-view RTS camera all run from the worker); c5b-2 (worker scene.pick selection + orders), c5b-3 (selection overlays), c5c (sceneState/audio/minimap bridges), and c6 (LuaUI world pass) pending.

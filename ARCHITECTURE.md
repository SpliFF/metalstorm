...
make build-release      # cmake --preset release + build (the binary the lobby spawns)
...
`make build` depends on `export-metalstorm-specs`, which needs a system `lua`
on PATH (it runs `tools/scripts/lua-to-json.lua` to mirror shared Lua spec
files into JSON for the client).

Gate targets (Makefile):

```
make test / test-cpp            # C++ suite: build spring-tests + ctest
make test-client                # cd client && npx vitest run
make test-all                   # test-cpp + test-client
make test-headless-batch        # headless-batch pure unit tests (node --test, no server build)
make test-headless-determinism  # determinism pair-run: fixture twice, diff stateHash sequences
make determinism-gate           # the multi-arm determinism gate (tools/headless-batch/determinism-gate.sh)
make test-replay-verify         # replay re-execution verify arm
make test-replay-spectate       # replay spectate arm
make soak-growth / soak-churn   # long-uptime soak arms (PLAN-long-uptime)
make build-release              # release binaries (build/release)
```

The determinism/persistence test substrate is `tools/headless-batch` (matrix
runner + fixtures), `spring-server --resume-verify` (capture → restore →
re-capture equality), and `tools/scripts/hibernate-resume-recapture.sh`
(SIGTERM-driven hibernate/resume round-trip).

`npx vitest run` from `client/` covers two module trees, because
`client/vite.config.ts` declares them as vitest **projects**:
`client` (`src/**/*.test.ts` + `wire/**/*.test.ts`) and `metalstorm-game`
...
```
./spring-lobby --port 8011 --db data/spring-server.db --games-dir data/games
```

There is **no `--maps` flag**: the maps directory derives from the `--db`
path's parent (`<db dir>/maps`). The flags shown here are representative, not
exhaustive — see `rts/lobby_main.cpp` for the full set (dev-direct-start,
idle/hibernate tuning, scenario knobs, `--modoption`, …).
...
Common CLI flags (from `rts/server_main.cpp`):
`--port`, `--game`, `--game-version`, `--map`, `--db`, `--log-file`, `--log-level`, `--log-server`, `--log-sqlite`, `--debug`, `--no-cache`, `--log-messages`, `--wt-cert <path>`, `--wt-key <path>` (WebTransport prod cert — see below), `--player username:team:pos` (repeatable), `--ai id:team:pos` (repeatable). Roughly 25 more flags exist in `rts/server_main.cpp` — the important families: headless runs (`--headless-run`, `--max-wall-min`), persistence (`--resume`, `--resume-verify`, `--no-hibernate`, `--snapshot-roundtrip`, `--roundtrip-strict`), replay/journal (`--replay*`, `--journal*`), room shape (`--session-kind`, `--room`, `--player-slots`, `--war-side-capacity`), plus `--verify`, `--modoption`, `--print-engine-hash`. The IPC pipe and `--event-fd` are gone; a game→lobby backchannel (PLAN-archive/PLAN-lobby-game-connection.md) is **not yet implemented** (TODO — targets WebTransport when built).
...
| `Server/TerrainPages.h` | Header-only producer for the client's ground-page pyramid (PLAN-maps §1.2.1, `MAP_FORMAT_VERSION` 19). Mirrors `terrain-page-grid.ts`'s address space texel for texel — 512² payload + a 4-texel (one BC1 block) border = 520² physical, 135 200 bytes — and cuts a map-space ground albedo into `ground_pages.bin` + the self-describing `ground_pages.json` index. **Only the levels the source resolution covers are produced** (the generator default is a 4096² source since the M8 streaming-v2 resolution raise, so a 16 384-elmo map gets levels 2..5 = 85 pages ≈ 11.5 MB; the pre-raise 2048² shape was levels 3..5 = 21 pages ≈ 2.8 MB and older packages keep working — the index self-describes); the client clamps its descent to `finestLevel` rather than being handed upsampled blur, and a source-resolution change is never a format change. BC1 is the same range-fit `terragen/dxt1.py` uses for SMT tiles. `MapProcessor::ExtractGroundTexture` invokes it through `textureconverter --terrain-pages` on the **source** image (resolved before `ConvertMapTexture` rewrites the field to `ground.ktx2`); a failure there is a degradation, not an error — the map still ships `ground.ktx2` and streaming is simply unavailable for it. `tests/test_terrain_pages.cpp` and `client/src/core/terrain-page-http.test.ts` pin the **same** byte offsets from the two sides, so a change that moves one suite and not the other is a broken on-disk contract. |
...
### World layer (persistent metagame, `rts/Server/World*.{h,cpp}`)

The persistent world layer (PLAN-worldsim W1–W12, landed 2026-08). **Hard
boundary: the world layer never touches sim state, and no `world_*` table is
keyed by `room_id`.** A war IS a room (`rooms.session_kind = 'persistent'`)
plus a `wars` row keyed by `room_id`; the war state machine is
seeding → open → active → winding_down → resolving → archived
(`Server/WarDirector.h`). Forces cross the seam in exactly one place each way:
arrivals enter a battle through `game_transports`, departures leave the same
way (see the world↔battle force seam below).

| File | Purpose |
|------|---------|
| `Server/WorldDirector.h/.cpp` | Orchestrator: owns the world row, wires the modules below, drives their ticks. |
| `Server/WorldClock.h` | World time as a **pure function** of (epoch, ratio, pause ledger, now). Default ratio 24:1 (one real hour = one world day); `world_pause_ledger` freezes everything measured on this clock. |
| `Server/WorldMapSeeder.h/.cpp` | Seeds a world's POI graph (`worlds`, `world_pois`, `world_poi_edges`). |
| `Server/WorldWarLinkage.h` | The war↔world join: which POI a war contests, read through the `wars` row. |
| `Server/WorldEconomy.h/.cpp` | Event-sourced treasury — **no balance column anywhere**; the ledger IS the balance, and a read prices the whole elapsed gap in closed form (income linear, decay `pow`-shaped) rather than looping world-days. |
| `Server/WorldFactions.h/.cpp` | Player-founded factions cast from **4 archetype sheets** (defaults copied at founding, then owned by the faction). `world_faction_members` is the **sole membership authority**; `side_key` is the only bridge to `users.faction_id` (a SIDE key, not an organisation). |
| `Server/WorldStaging.h/.cpp` | Commitment windows (`world_staging`); materialisation goes through the existing `demandSeedWar` path in `lobby_main.cpp` — the world layer never creates a room itself. |
| `Server/WorldStats.h/.cpp` | The public stats digest (`/api/world/stats`); settles authority accrual idempotently on the way past. |
| `Server/WorldSeasons.h/.cpp` | Seasons: 14 world-days each (`world_seasons`); rollover archives a digest (`world_season_digests`) computed over the two ledgers (force + settlement). |
| `Server/WorldNotifications.h/.cpp` + `Server/WorldOfflineChannels.h/.cpp` | Offline delivery: Discord webhook + Web Push (RFC 8291, `world_push_subscriptions`). Default-disabled; bounded queue, drops rather than blocks. |
| `Server/WorldEscrow.h/.cpp` | Committed forces: committed → engaged → settled, outcomes `held | withdrew | routed | annihilated`; `annihilatedCaptureFraction` 0.25 (an annihilated expedition is a TRANSFER — comeback pressure); settlement is exactly-once into `world_force_ledger` (capture-then-claim order). |
| `Server/WorldConquest.h/.cpp` | Ownership changes by **explicit claim act** (`world_poi_claims`), never as a war side-effect: defender's shield, earliest-open-winning-claim tie-break on the world clock, 30 world-day expiry (admin pause freezes it). |

**World↔battle force seam:** `data/games/metalstorm/LuaRules/Gadgets/game_transports.lua`
(sim side) + `data/games/metalstorm/client/squads/squad-transport.js` (client
side). Arrivals via `game_transports` are the only way forces enter a battle;
...
| File | Purpose |
|------|---------|
| `main.ts` | App entry. Lobby init, `startGame()`, HUD wiring. **GW4-c1**: `startGame()` now transfers `#game-canvas` to the game-processor worker (`gp:init`) and spawns it; the Babylon Engine + render loop + game connection live in the worker, not here (PLAN-archive/PLAN-game-worker.md). |
| `config.ts` | Server URL, API base paths. |
| `core/connection.ts` | WebTransport game-stream connection to server (over `WebTransportAdapter`). FlatBuffers dispatch. Events: `onMapData`, `onUnitDefs`, `onEntityState`, `onCombatEvents`, etc. GW4 relocates it into the game-processor worker (PLAN-archive/PLAN-game-worker.md). |
| `core/transport.ts` | Transport abstraction over the game connection. `WebTransportAdapter` (QUIC/HTTP-3) — class-based send (`control`/`state`/`vision`/`bulk`/`datagram`), newest-wins state. WebRTC removed (PLAN-archive/PLAN-game-worker.md). |
| `core/game-worker-protocol.ts` | **Frozen GW4 message contract** (PLAN-archive/PLAN-game-worker.md): the game-processor worker ⇄ main-thread interfaces (`Gp*ToWorker` init/input/config, `Gp*ToMain` sceneState/audio/config/gameOver). Also (WP2c) `LegacyWorkerMessage` (all legacy `type` strings) + `WorkerInbound` union used to type `self.onmessage`. |
...
| `core/atmosphere.ts` | L-ATMOS: procedural sky dome (`SkyMaterial`) + `scene.fogMode`/`clearColor`, driven by `mapinfo.lua atmosphere` (`MapAtmosphere`, `map-lighting.ts`) and the sun direction. `gfx.sky` gates the dome; low preset also drops fog to `FOGMODE_LINEAR`. See docs/lighting.md "Atmosphere". |
| `core/water-surface.ts` | L-ATMOS: the Y=0 water plane material — scrolling analytic bump fields + Fresnel + shore foam (reuses `water-absorption-plugin.ts`'s depth-blend curve), replacing the flat `StandardMaterial` stand-in. Not `WaterMaterial` (no reflection RTT). |
...
| `core/terrain-page-grid.ts`, `core/terrain-page-visibility.ts`, `core/terrain-page-cache.ts` | **Opt-in only** (the `__terrainPages` debug handle) — the non-GPU core of PLAN-maps §1.2.1 ground-texture page streaming. `terrain-page-grid` is the address space (512² payload + 4-texel border = 520² BC1 pages, a mip pyramid where the pyramid *is* the mip chain, packed page keys, parent chain, global-map-UV → page-UV transform, `MAX_ARRAY_TEXTURE_LAYERS`-clamped layer budget). `terrain-page-visibility` is the CPU visible set — a quadtree descent culling each page's box with `shadow-depth-bounds.ts`'s `viewDepthRangeOfBox` × `HeightRangeGrid.rangeOverRect` (frustum ∩ heightfield; **no GPU feedback readback, deliberately**). `terrain-page-cache` is LRU residency over `TEXTURE_2D_ARRAY` layers with a pinned root, Cesium-style priority groups, `AbortController` cancellation, the page-table indirection texture + cross-fade, a Cache API disk tier, and the `PageSource`/`PageUploader` seams. `VisiblePagesOptions.minLevel` floors the descent at the producer's `finestLevel` so no page finer than the source is ever requested. **DIVERGENCE:** the eventual uploader uses raw-GL `compressedTexSubImage3D` — Babylon has no public compressed 2D-array sub-image API. |
| `core/terrain-page-gl.ts`, `core/terrain-page-plugin.ts`, `core/terrain-page-synthetic.ts`, `core/terrain-page-streaming.ts` | The GPU half of §1.2.1 streaming (vertical slice, **opt-in via the `__terrainPages` debug handle only** — `enable()` prefers the map's real pages, `enable({synthetic:true})` forces the hue-per-level diagnostic source). `terrain-page-gl` carries the recorded raw-GL divergence: `TEXTURE_2D_ARRAY` allocation (`texStorage3D`, no internal mips) + `compressedTexSubImage3D` per-layer fills with `buildAtlasPage`'s drain-then-check GL-error discipline, adopted back via `Engine.wrapWebGLTexture` + `is2DArray`; also the RGBA8 page-table texture, re-uploaded only on `cache.revision` change. `terrain-page-plugin` is `TerrainPageSamplePlugin` (priority 180, before splat/decal/water): page-table `texelFetch` → two array taps → `mix(fallback, primary, fade)`; toggle is **`isEnabled`**, and the attach helper asserts the setter took. `terrain-page-streaming` drives visible set → cache → table sync per frame off `scene.onBeforeRenderObservable`. The plugin survives `terrain.ts`'s material swaps via the same find/reattach dance as the splat plugin. |
| `core/terrain-page-http.ts` | The **real** `PageSource` (PLAN-maps §1.2.1, `MAP_FORMAT_VERSION` 19). Discovery is a file convention, not a protocol field: `GET <mapDataUrl>/ground_pages.json` — a 404 means "this map ships no pages" (every real Spring map, whose ground colour still comes through the SMT tile dictionary). The index is **validated against the client's own `planPageGrid`** before a single byte is sampled — a mismatch refuses to stream rather than sampling garbage. One page is one HTTP **Range** request over `ground_pages.bin` (135 200 bytes, offsets mirroring `TerrainPages::PageByteOffset`); a server that ignores `Range` and answers 200 degrades to fetch-whole-file-once + slice locally. `sourceId` carries the producer stamp so `withPageDiskCache` cannot serve pages from a previous reprocess. |
...
| `core/lua-widget-worker.ts` | **Worker entry point** (must stay at this path — `main.ts` `?worker`-imports it by name; the retired `lua-widget-manager.ts` used to bundle it identically). After WP2c this file is thin: imports from `game-processor.ts` + `lua-ui-host.ts` + `gp-context.ts` and the typed `self.onmessage` dispatcher. All logic lives in the sub-modules. |
...
| `ui/native-ui/tokens.css` | **The design tokens of record** — the whole `--nui-*` palette, type (`@font-face`, self-hosted OFL woff2 in `client/public/fonts/`) and surface texture, from `data/games/metalstorm/art/DIRECTION.md`. Linked by `client/index.html` (not `?raw`-injected, so the injected sheets can reference it); no other stylesheet may name a colour (`tokens.test.ts`). |
| `ui/native-ui/native-ui.css` | **The native-UI design system** — dock/rail geometry (`.ui-mount-*`), panel chrome (`.nui-panel__head/__body/__badge`, `.is-collapsed`), and control primitives (`.nui-btn/-field/-row/-list/-table/-meter/-badge/-chip/-menu/-toast`). Injected once by the widget-loader. |
...
| `ui/native-ui/nl-*.ts` + `query-engine.ts` + `voice-capture.ts` + `reveal-predicate.ts` | **The NL command surface** (drill-down UI directive): `nl-client`/`nl-envelope`/`nl-resolver`/`nl-executor`/`nl-clarify`/`nl-local-patterns`/`nl-context`/`nl-schema` parse and execute natural-language commands against the current focus; `query-engine.ts` answers state queries; `voice-capture.ts` is the mic input; `reveal-predicate.ts` is the drill-down `revealOn` grammar (what makes a hidden panel surface itself). Offline eval harness: `tools/nl-eval`. |
...
- **Lua bridge array marshalling:** a plain JS array returned to Lua spreads as N return values, NOT a table — `ipairs()` on the "return" sees nil. Wrap in `luaTable()` **even when the array is empty** (an empty array marshals as zero return values). The long-standing unwrapped case — `ordersToLuaArray` (`GetUnitCommands`/`GetFactoryCommands`/`GetCommandQueue`) — was fixed 2026-08-27 (`f614afbb14`, all bindings audited), but the rule still binds every NEW binding: wrap bare arrays with `luaTable()`.
...
**Wire format:** Every binary frame (over WebTransport — PLAN-archive/PLAN-game-worker.md) starts with `u8 envelope`:
...
ahead of the dispatch switch rather than per case. Exactly three of the 46
...
`mapinfo.lua → metalstorm = { reachability = "connected" | "split" }` is the map's own statement about whether armour can reach every start position from every other. The engine never reads it; `tools/mapgen/regions_from_map.py --verify` does, and judges the connectivity it measures off the passability mask against what the map declares. A split or island map is legal player content (PLAN-maps.md §2k), so the measurement alone is not a verdict — but the check is two-sided: an undeclared map that comes out split fails, and a map declaring `split` that comes out connected fails as a stale declaration. A start position with no passable ground near it at all is refused under both. `terragen/reachability.py` owns the vocabulary, the emitter every generated package writes, the parser, and the verdict; the intent is authored per map id in the generators (`archipelago.SHIPPED_REACHABILITY`, `meridian2.DECLARED_REACHABILITY`), never derived from the terrain a run produced. Since M9o's find the declaration also names its **scope** — `reachability_classes = "VEH HEAVY"` — the movement classes the claim speaks for: reachability is per CLASS while the claim is per MAP (meridian is split for armour and connected for INFANTRY, on purpose — the scarp is inside infantry's 45° maxslope), so `--verify` judges the declared intent only for a class inside the scope and falls back to the strict default outside it. An absent key (any pre-scope package) reads as the gate classes (`VEH`), which is what those packages were in fact judged on. The sibling gate for machine-run wars is `scenariogen.gate_reachability`'s `mutual` flag.
...
The census above is the pre-war/pre-world core. The rest of the
`spring-server.db` schema, grouped:

**Wars** (every `war*` table is keyed by `room_id` — a "war" is one battle):
`wars` (the war row + state machine), `war_sides` (per-side declaration/capacity),
`war_outcome` (terminal verdict), `war_player_bindings` (one row per war-room ×
account — the seat authority), `war_slot_reservations` (transactional seat
holds), `war_reconnect_tokens` (long-TTL per-war reconnect keys).

**Auth/session extras:** `refresh_tokens` (rotating refresh families),
`user_totp` + `user_totp_recovery` (optional second factor), `guest_devices`
(provisional-account device binding).

**Player journey:** `users.standing` / `sessions_played` / `callsign` / `commander_kind` /
`intro_done` (probe+ALTER columns), `mentorships` + `mentor_endorsements`.

**Social:** `friend_edges` (friendship graph), `chat_messages` (chat history),
`chat_mutes` (timed/account mutes; `chat_ignores` is the per-user ignore list).

**Persistence:** `game_snapshots` (GameStateStore's durable snapshot rows —
hibernate/checkpoint/rollback history).

**World layer** (no `world_*` table is keyed by `room_id`): `worlds` (world
row + clock epoch/ratio), `world_pois` + `world_poi_edges` (the POI graph),
`world_poi_claims` (explicit conquest claims), `world_factions` +
`world_faction_members` (player-founded factions; members is the sole
membership authority), `world_commanders` (per-account world presence),
`world_authority` + `world_authority_events` (per-account standing, event-
sourced), `world_economy_cursor` + `world_economy_events` (event-sourced
faction treasuries — no balance column), `world_escrow` (committed forces),
`world_force_ledger` (the force ledger settlements pay into),
`world_settlement_ledger` (exactly-once settlement rows), `world_staging`
(commitment windows), `world_seasons` + `world_season_digests` (14-world-day
seasons + archived digests), `world_pause_ledger` (world-clock pause
intervals), `world_push_subscriptions` (Web Push endpoints).
...
Realistic maps are generated, not hand-authored: `tools/mapgen` drives the
`terragen/` package (uplift/erosion/climate/biomes/roads/bridges → SMF bake).
See [docs/maps/generation.md](docs/maps/generation.md). Reachability is
declared per map (`metalstorm.reachability`) and enforced by test scenarios.

### Model production (`tools/forge`)

Production unit/building models come from the pre-built fable-model-forge
environment at `tools/forge` — see
[tools/forge/docs/FORGE-GUIDE.md](tools/forge/docs/FORGE-GUIDE.md). The
...
| `GET /api/world/seasons/*` | Public. One season by number plus its archived digest rows; an ACTIVE season answers 200 with empty `digests`. |
| `POST /api/world/pause` | AdminOnly. `{"action":"pause"\|"resume","reason"?}` — freezes world-clock progression only (in-flight battles keep running). |
| `GET /api/world/factions` | Public. Faction roster + the archetype catalogue + founding rules. |
| `POST /api/world/me` | TokenRequired. This account's standing in one world: world authority, founding-gate status, membership. POST so the dispatch gate sees the token. |
| `POST /api/world/factions/found` | TokenRequired. Found a faction. The founder's side key is read off the account, never the body. |
| `POST /api/world/factions/join` | TokenRequired. `{"factionId"}` — where the `users.faction_id` (side) seam is enforced. |
| `POST /api/world/factions/leave` | TokenRequired. Leave this world's faction; `users.faction_id` is deliberately NOT cleared (the side outlives the organisation). |
| `GET /api/world/push/key` | Public. Whether the world offers Web Push + the VAPID public key. |
| `POST /api/world/push/subscribe` | TokenRequired. Store a browser PushSubscription; insert-or-replace on (world, account, endpoint). |
| `POST /api/world/push/unsubscribe` | TokenRequired. `{"endpoint"}` — scoped to the acting account. |
| `POST /api/world/staging/commit` | TokenRequired. Commit forces into a staging window; the attacking faction is read from the caller's membership, never the body. |
| `POST /api/world/staging/cancel` | TokenRequired. Call a commitment off — authorised against the committing FACTION (any member), not the committing account. |
| `GET /api/world/claims` | Public. Every claim + the rates in force (a filed claim is declared strategic map information). |
| `POST /api/world/claims/file` | TokenRequired. File a conquest claim; the claiming faction is the caller's own membership, the charge the caller's own authority. |
| `POST /api/world/claims/withdraw` | TokenRequired. `{"claimId"}` — any member of the claiming faction may withdraw; the refund returns to the account that paid. |
| `GET /api/world/stats` | Public. World stats digest. Read-only client-side; settles authority accrual idempotently on the way past (W8 — W9 owns the tick). |
...
(The nine `/api/chat/*` routes are registered in `rts/lobby_main.cpp` and live
in the lobby table above.)
...
PLAN-archive/PLAN-lobby-game-connection.md.
...
`POST /api/account/me` and `POST /api/account/profile` (registered by
`HttpAuth::RegisterAccountRoutes`, lobby-only) are the account surface and **gate
nothing** — `me` is a read of the caller's own row with tier derived, `profile`
writes the three cosmetic fields independently. Both are POST including the read:
a GET handler here is not handed the request headers, so it cannot learn which
account the dispatcher admitted (the reason `/api/friends/list` states).
`POST /api/auth/guest` takes an optional `username`, validated by the one
`GuestAccounts::ValidNickname` rule and answered `409 {name_taken:true}` when a
registered account holds it — guest nicknames share the `users.username`
namespace, which is what stops a guest spectating under a player's name.
...
server tick (server_main.cpp)
  BeginTick(frame)
  1 Inbound     DrainInbound  → ClientMessageHandler::HandleMessage   ← records ALL 46 wire verbs
...
the proxy capsule). Nothing in that chain is an error. Placeholders are no
longer the norm — the `wz_*` stand-ins were replaced by the first-party forge
`ms_*` corpus (only `units/wz_baseline.lua`'s test defs still reference them) —
but the mechanism is unchanged, so a def whose `objectname`
regresses would still degrade with a clean render and only the WARNING below
to say so. `FindDefsWithMissingModels` closes the hole: once per defs bake the
...
**The game is Metalstorm.** BAR and Zero-K were archived 2026-08-02
(PLAN-archive/ARCHIVED-2026-08-02-bar-zk.md); a running Metalstorm demo is the
primary focus, and the named work focus is **mission objectives + transports**.

**Stable end-to-end loop:** lobby → create room → start game → fight → game-over → return to lobby, with disconnect handling, dynamic join, and war resume. Transport: HTTP/2 (h2c) + HTTP/1.1 for REST/SSE/assets; game-state streaming over WebTransport (QUIC/HTTP-3).

**Landed and load-bearing:**
- **Persistent world layer** (PLAN-worldsim W1–W12): merges `25ca7c6d18` + `7addf25a24`, conquest `0140dfc2d1`, escrow `84cb40b198`, seasons/notifications `592f07e497`. See the World layer section in the Directory Map and the `/api/world/*` routes.
- **The 2026-08-19 ruling:** in-battle construction is **field engineering only** — base building and the economy live in the world layer. Non-building factories in a battle are not a defect.
- **World scale: 8 elmos = 1 m**, applied at model import (the whole corpus rescaled ×8, 2026-08-27); `tools/scripts/check_model_scale.py` is the gate.
- **SoA squad engine is the default** (`config.engine:'soa'`, `data/games/metalstorm/client/squads/`).
- **Terragen map pipeline** (`tools/mapgen` + `terragen/`, docs/maps/generation.md) with per-map declared reachability contracts.
- **Drill-down UI directive (2026-08-29):** the battle UI stays out of the way until needed — panels surface on `revealOn` predicates, and the NL command surface reads the current focus. Governs all battle-UI work.
- **Headless determinism gate is green**, and exit codes are honest again: the abort that made every headless run exit 134 was fixed 2026-08-27 (`f10db92c0a`) — success exits 0, so exit-code gating is safe; the verdict line remains the richer signal.
- **Unit-art corpus complete:** the `wz_*` placeholders were replaced by the first-party forge `ms_*` model corpus.

**Not yet wired:**
- **Glicko-2 ratings:** `rts/Server/RatingSystem.h/.cpp` implements Glicko-2, but nothing references it — no route, room path, or game-over flow reads or writes a rating.
- **Victory/defeat music:** the GameOver → `MusicStateTracker::ForceState(victory|defeat)` hook is the one remaining audio gap — the rest of the pipeline (state machine, MusicEvent broadcast, client crossfader) is live.
- **Game→lobby backchannel** (PLAN-archive/PLAN-lobby-game-connection.md): still not implemented; when built it targets WebTransport.
